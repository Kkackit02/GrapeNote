...
export async function setGrapeSkin(skinId: string): Promise<ActionResult> {
  ...
  const [{ data: cardRows }, { data: subRows }] = await Promise.all([
    ...
    supabase.from("submissions").select("status, created_at, instrument").eq("student_id", user.id),
  ]);
  const cards = (cardRows ?? []) as Pick<ProgressCard, "completed_at">[];
  const subs = (subRows ?? []) as Pick<Submission, "status" | "created_at" | "instrument">[];
  const stats: SkinStats = {
    ...
    grapesByInstrument: tallyGrapesByInstrument(subs),
  };
