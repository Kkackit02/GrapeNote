...
-- (모든 객체를 public. 으로 명시 — SQL Editor의 search_path 상태와 무관하게 동작)
...
create index if not exists song_tracks_song_idx on public.song_tracks(academy_id, song_title);

alter table public.song_tracks enable row level security;

-- 같은 학원(그룹)이면 모두 듣는다
create policy tracks_select on public.song_tracks for select
  using (academy_id = public.auth_academy_id());

-- 본인 명의 + 우리 학원 + 경로 접두사 강제 (남의 파일 경로 등록 차단)
create policy tracks_insert on public.song_tracks for insert
  with check (
    academy_id = public.auth_academy_id()
    and uploaded_by = auth.uid()
    and file_path like public.auth_academy_id()::text || '/tracks/%'
  );

-- 내 것은 내가, 선생님은 전부 정리 가능
create policy tracks_delete on public.song_tracks for delete
  using (
    uploaded_by = auth.uid()
    or (academy_id = public.auth_academy_id() and public.auth_role() = 'teacher')
  );
